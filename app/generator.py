...
def generate_documents(data: dict):
    ...
    word_manif = os.path.join(OUTPUT_DIR, f"{base}_manifestacion.docx")
    word_cert = os.path.join(OUTPUT_DIR, f"{base}_certificacion.docx")
    pdf_path = os.path.join(OUTPUT_DIR, f"{base}_completo.pdf")

    generate_word_manifestacion(data, word_manif)
    generate_word_certificacion(data, word_cert)
    generate_pdf(data, pdf_path)

    zip_path = os.path.join(OUTPUT_DIR, f"{base}_word.zip")
    with zipfile.ZipFile(zip_path, "w") as zf:
        zf.write(word_manif, os.path.basename(word_manif))
        zf.write(word_cert, os.path.basename(word_cert))
